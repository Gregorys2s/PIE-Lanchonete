...
import com.github.Gregorys2s.model.repositories.CardapioRepository;
...
import com.github.Gregorys2s.model.repositories.RelatorioDiarioRepository;
...
public class AppConfig {
    ...
    public AppConfig() {
        ...
        PagamentoRepository pagamentoRepository =
                new PagamentoRepository(em);

        PagamentoService pagamentoService =
                new PagamentoServiceImpl(pagamentoRepository);

        // ===== CARDÁPIO =====
        CardapioRepository cardapioRepository =
                new CardapioRepository(em);

        CardapioServiceImpl cardapioService =
                new CardapioServiceImpl(cardapioRepository);

        this.cardapioController =
                new CardapioController(cardapioService);

        this.cardapioView =
                new CardapioView(cardapioController);

        // ===== PEDIDOS =====
        PedidosRepository pedidosRepo =
                new PedidosRepository(em);

        PedidosService pedidosService =
                new PedidosServiceImpl(
                        pedidosRepo,
                        pagamentoService,
                        caixaService
                );

        this.pedidosController =
                new PedidosController(pedidosService);

        // ===== INGREDIENTES =====
        IngredienteRepository ingredienteRepository =
                new IngredienteRepository(em);

        IngredientesServiceImpl ingredientesService =
                new IngredientesServiceImpl(ingredienteRepository);

        this.ingredientesController =
                new IngredientesController(ingredientesService);

        // ===== RELATÓRIO DIÁRIO =====
        RelatorioDiarioRepository relatorioDiarioRepository =
                new RelatorioDiarioRepository(em);

        RelatorioDiarioServiceLmpl relatorioDiarioService =
                new RelatorioDiarioServiceLmpl(
                        relatorioDiarioRepository,
                        pedidosRepo
                );

        this.relatorioController =
                new RelatorioController(relatorioDiarioService);
